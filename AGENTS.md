...
- **USB SPI команды**: stream/bench — `SPI_STREAM_REAL`, `SPI_STREAM_REAL_SLOT`, `SPI_STREAM_REAL_FAST`, `SPI_STREAM_REAL_LEGACY`, `SPI_STREAM_RR8_REAL`, `SPI_RATE_RR8`, … Host: `python3 tools/usb_spi_rr8_bench.py`. `SPI_STREAM_REAL` сейчас использует slot-DMA (`TIM1_CH2` CS на PE11 + `TIM1_UP` TX DMA + `SPI2_RX` DMA), period **44 SCK cycles**, CS-high **300 ns**, и перед стартом делает одноразовый `CONVERT H=1` для сброса DSP HPF. `SPI_STREAM_REAL_FAST` — регистровый polling, `SPI_STREAM_REAL_LEGACY` — старый свободно бегущий TIM+DMA CS path.
