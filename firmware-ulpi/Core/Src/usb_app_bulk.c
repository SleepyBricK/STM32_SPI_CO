...
#define USB_STREAM_MAX  65535U
#define USB_STREAM_PKT  (VENDOR_BULK_HS_MAX_PACKET / 2U)
#define USB_TX_TIMEOUT  1000U

static uint8_t s_rx[USB_CMD_MAX];
static char s_reply[USB_REPLY_MAX];
static uint16_t s_stream_buf[USB_STREAM_PKT];
...
static HAL_StatusTypeDef wait_usb_tx_ready(void)
{
  ...
  {
    if ((HAL_GetTick() - t0) > USB_TX_TIMEOUT)
    {
      return HAL_TIMEOUT;
    }
  }
...
static HAL_StatusTypeDef stream_ramp_samples(uint32_t n)
{
  uint32_t sent = 0U;
  ...
  while (sent < n)
  {
    ...
    if (block > USB_STREAM_PKT)
    {
      block = USB_STREAM_PKT;
    }

    for (uint32_t i = 0U; i < block; i++)
    {
      s_stream_buf[i] = (uint16_t)((sent + i) & 0xFFFFU);
    }

    while (off < block)
    {
      uint32_t chunk = block - off;
      uint16_t bytes;

      if (chunk > USB_STREAM_PKT)
      {
        chunk = USB_STREAM_PKT;
      }

      bytes = (uint16_t)(chunk * 2U);
      if (wait_usb_tx_ready() != HAL_OK)
      {
        return HAL_TIMEOUT;
      }
      if (USBD_VENDOR_BULK_Transmit((uint8_t *)&s_stream_buf[off], bytes) != USBD_OK)
      {
        return HAL_ERROR;
      }
      off += chunk;
...
static void dispatch_usb_command(char *line)
{
  ...
  {
    const char *usb33 = (__HAL_PWR_GET_FLAG(PWR_FLAG_USB33RDY) != 0U) ? "1" : "0";
    (void)snprintf(s_reply, sizeof(s_reply),
                   "OK FW=ulpi-v1 USB33RDY=%s TX=%u\n",
                   usb33, (unsigned)USBD_VENDOR_BULK_TxReady());
    s_reply_len = (uint16_t)strlen(s_reply);
    return;
  }
  ...
  if (strcmp(cmd, "STREAM") == 0)
  {
    ...
    if (n == 0UL || n > USB_STREAM_MAX)
    {
      set_reply("ERR n");
      return;
    }

    if (stream_ramp_samples((uint32_t)n) != HAL_OK)
    {
...
void USB_App_Bulk_Process(void)
{
  uint16_t len = 0U;
