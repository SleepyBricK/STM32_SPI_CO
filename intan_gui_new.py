...
class IntanGuiApp(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        help_text = """STM32 intan_pattern.c: до 1024 слотов, 1 CS на PATTERN_ADD_RAW.
Импульс: ON → delay → raw OFF → delay (последний без delay) → PATTERN_ADD_WRITE 42 0 1 0.
§1 INIT_STIM/send_line → pattern_load → pattern_run. Прошивка: авто-OFF после RUN."""
        ttk.Label(
    ...
    @classmethod
    def build_stim_prep_send_lines(cls):
        """§1 Подготовка стима (send_line → STM32 USB, вне pattern_load)."""
        return ["INIT_STIM", "WRITE 42 0 1 0", "CLEAR_COMP"]

    @classmethod
    def build_stim_safety_off_line(cls):
        """Полный WRITE R42=0 U=1 — 3 CS-слота; надёжнее raw OFF в конце паттерна."""
        return "PATTERN_ADD_WRITE 42 0 1 0"

    @classmethod
    def _finalize_pattern_load_lines(cls, load_lines):
        """Убирает delay после последнего импульса; добавляет PATTERN_ADD_WRITE OFF."""
        out = [ln.split("#", 1)[0].strip() for ln in load_lines if ln.split("#", 1)[0].strip()]
        while out and out[-1].upper().startswith("PATTERN_ADD_DELAY"):
            out.pop()
        if not any(ln.upper().startswith("PATTERN_ADD_WRITE 42") for ln in out):
            out.append(cls.build_stim_safety_off_line())
        return out

    @classmethod
    def _write_line_to_pattern_add_raw(cls, line: str) -> str | None:
        """Конвертирует WRITE reg val [U] [M] → PATTERN_ADD_RAW (1 SPI-слот)."""
        cmd = line.split("#", 1)[0].strip()
        if not cmd:
            return None
        parts = cmd.split()
        if parts[0].upper() != "WRITE" or len(parts) < 3:
            return None
        try:
            reg = int(parts[1], 0)
            value = int(parts[2], 0)
        ...
        u_flag = "U" in parts
        m_flag = "M" in parts
        word = cls._intan_raw_word(reg, value, u_flag=u_flag, m_flag=m_flag)
        return f"PATTERN_ADD_RAW 0x{word & 0xFFFFFFFF:08X}"
    ...
    def _pattern_lines_for_load(cls, prep_lines, pattern_cmd_lines, *, finalize=True):
        ...
        for ln in prep_lines + pattern_cmd_lines:
            ...
            if cmd_type == "DELAY" and len(parts) >= 2:
                out.append(f"PATTERN_ADD_DELAY_CYC {parts[1]}")
                continue
            if cmd_type == "WRITE" and len(parts) >= 3:
                raw_line = cls._write_line_to_pattern_add_raw(ln)
                if raw_line:
                    out.append(raw_line)
                continue
            if cmd_type in (
    ...
    @classmethod
    def estimate_stm32_pattern_slots(cls, load_lines, include_server_safety_off=False):
        ...
        for ln in load_lines:
            ...
            if cmd_type == "PATTERN_ADD_RAW":
                slots += 1
                spi += 1
            elif cmd_type in ("PATTERN_ADD_DELAY_US", "PATTERN_ADD_DELAY_CYC"):
                slots += 1
                delays += 1
    ...
    def _run_stim_prep_sync(self):
        """INIT_STIM + safe state перед pattern_load (send_line → USB STM32)."""
        for line in self.build_stim_prep_send_lines():
            self.client.send_line(line, timeout=TCP_TIMEOUT_SEND_LINE)

    def _pattern_load_sync(self, prep_lines, pattern_cmd_lines):
        """Загрузка паттерна в STM32 (pattern_load)."""
        pattern_load_lines = self._pattern_lines_for_load(prep_lines, pattern_cmd_lines, finalize=True)
    ...
    def on_pattern_load(self):
        ...
        if not load_lines:
            ...
            return
        slot_est, spi_est, delay_est = self.estimate_stm32_pattern_slots(load_lines, include_server_safety_off=False)
        ...
        def worker():
            try:
                self.log("Подготовка стима (INIT_STIM)...", "info")
                try:
                    self._run_stim_prep_sync()
                    self.log("✓ INIT_STIM / CLEAR_COMP", "success")
                except Exception as prep_err:
                    self.log(
                        f"⚠ Prep не выполнен: {prep_err} "
                        "(нужен intan_server --backend usb с send_line INIT_STIM)",
                        "warning",
                    )
        ...
        for i, line in enumerate(pattern_lines):
            ...
            if cmd_type == "WRITE":
                ...
                if len(parts) >= 4:
                    try:
                        commands.append({
                            "type": "PATTERN_ADD_WRITE",
                            "line": i + 1,
                            "reg": int(parts[1], 0),
                            "value": int(parts[2], 0),
                            "u": "U" in parts or (len(parts) >= 4 and parts[3].upper() in ("1", "U")),
                        })
                    except Exception:
                        commands.append({"type": "error", "line": i+1, "text": line})
    ...
    def update_pattern_visualization(self):
        ...
        for cmd in commands:
            ...
            if cmd_type == "WRITE":
                ...
                text = f"WRITE R{cmd['reg']} = 0x{cmd['value']:04X}"
                if cmd.get("u"):
                    text += " U"
            elif cmd_type == "PATTERN_ADD_DELAY_US":
                text = f"DELAY_US {cmd['delay_us']} µs"
...
• DELAY_US <µs> - задержка в микросекундах (PATTERN_ADD_DELAY_US)

• PATTERN_ADD_RAW — ON/OFF R42/R44 (1 CS на слот)

• PATTERN_ADD_DELAY_US — пауза µs

• PATTERN_ADD_WRITE 42 0 1 0 — только safety OFF в конце (3 CS)

• Последний импульс: без delay после OFF; прошивка дублирует OFF после RUN
...
Начните вводить паттерн, и здесь появятся подсказки!"""
            ...
            for c in commands:
                if c.get("type") == "PATTERN_ADD_RAW":
                    ...
                    u = " U" if c.get("u") else ""
                    m = " M" if c.get("m") else ""
                    raw_line = self._write_line_to_pattern_add_raw(
                        f"WRITE {c['reg']} 0x{c['value']:04X}{u}{m}"
                    )
                    if raw_line:
                        load_lines.append(raw_line)
                elif c.get("type") == "DELAY_US":
                    load_lines.append(f"PATTERN_ADD_DELAY_US {c['delay_us']}")
                elif c.get("type") == "DELAY":
                    load_lines.append(f"PATTERN_ADD_DELAY_CYC {c.get('count', 0)}")
                elif c.get("type") == "PATTERN_ADD_WRITE":
                    u = " 1 0" if c.get("u") else " 0 0"
                    load_lines.append(f"PATTERN_ADD_WRITE {c['reg']} {c['value']}{u}")
            load_lines = self._finalize_pattern_load_lines(load_lines)
            slots_est, spi_est, delays_est = self.estimate_stm32_pattern_slots(
                load_lines, include_server_safety_off=False
            )
            raw_42_on = sum(
                1 for c in commands
                if c.get("type") == "PATTERN_ADD_RAW"
            ...
            if write_count > 0:
                hints += f"  • WRITE (→ PATTERN_ADD_RAW при load): {write_count}\n"
            if delay_us_count > 0:
    ...
    def format_guide_pattern_text(
        ...
        slot_est, spi_est, delay_est = self.estimate_stm32_pattern_slots(
            load_preview, include_server_safety_off=False
        )
        lines = [
            "# STM32 intan_pattern.c — intan_stim_pattern_guide.md",
            f"# Оценка load: ~{slot_est} слотов (spi≈{spi_est}, delays≈{delay_est}, max 1024)",
            "",
            "# §1 Подготовка (send_line перед «Загрузить паттерн»)",
        ]
        lines.extend(self.build_stim_prep_send_lines())
        lines.extend([
            "",
            f"# §2 pattern_load — ch{channel}, {current_ua} µA",
            "# Токи (PATTERN_ADD_RAW R64/R96):",
        ])
        lines.extend(pre_setup_lines)
        lines.extend(["", "# Импульсы: ON → delay → raw OFF → delay (последний без delay)"])
        lines.extend(pattern_lines)
        if not any("PATTERN_ADD_WRITE 42" in ln for ln in pattern_lines):
            lines.append(self.build_stim_safety_off_line() + "    # safety OFF")
        lines.extend([
            "",
            f"# §3 Запуск (кнопка «Запустить», repeat={int(repeat_count)})",
    ...
    def build_stim_pulse_slots(
        self, channel, pulse_us=100, pause_us=100, positive=True, *,
        include_polarity=False, trailing_pause=True,
    ):
        """Один импульс: R42 ON + delay + raw OFF + optional delay; R44 — один раз на паттерн."""
        channel = int(channel)
        ...
        if include_polarity:
            ...
            slots.append(self._format_pattern_add_raw(raw_pol, f"R44 polarity ch{channel}"))
        slots.extend([
            self._format_pattern_add_raw(raw_on, f"R42 ON ch{channel} U=1"),
            f"PATTERN_ADD_DELAY_US {int(pulse_us)}",
            self._format_pattern_add_raw(raw_off, "R42 OFF raw U=1"),
        ])
        if trailing_pause is not False:
            pause = int(pause_us if trailing_pause is True else trailing_pause)
            slots.append(f"PATTERN_ADD_DELAY_US {pause}")
        return slots

    def build_stim_duration_sweep(
        self, channel, durations_us=None, positive=True,
    ):
        """Sweep: по одному импульсу на каждую длительность; пауза = длительность."""
        if durations_us is None:
            durations_us = [500, 200, 100, 50, 20, 10]
        channel = int(channel)
        pol_mask = (1 << channel) if positive else 0
        slots = [
            self._format_pattern_add_raw(
                self._intan_raw_word(44, pol_mask, u_flag=False),
                f"R44 polarity ch{channel}",
            ),
        ]
        for i, us in enumerate(durations_us):
            slots.extend(
                self.build_stim_pulse_slots(
                    channel, us, us, positive,
                    include_polarity=False,
                    trailing_pause=(i < len(durations_us) - 1),
                )
            )
        slots.append(self.build_stim_safety_off_line() + "    # safety OFF (3 CS)")
        return slots

    def build_stim_pattern_body(self, channel, pulse_us=100, pause_us=100, positive=True):
        """Один импульс с R44 в начале (пример «📄 Пример»)."""
        return self.build_stim_pulse_slots(
            channel, pulse_us, pause_us, positive, include_polarity=True
        )

    def build_stim_postrun_commands(self, repeat_count=1):
        """§3 Запуск (прошивка также шлёт WRITE R42=0 после PATTERN_RUN)."""
        return [
            f"PATTERN_RUN {int(repeat_count)}",
            "READ 42",
            "READ 40",
    ...
    def on_pattern_load_example(self):
        ...
        pattern = self.build_stim_duration_sweep(channel)
        ...
        self.on_pattern_text_change()
        self.log(
            f"Загружен пример: ch{channel}, {current_ua} µA, sweep 500→10 µs",
            "info",
        )
    ...
    def create_block_widget(self, block):
        ...
        if block["type"] == "current":
            ...
            ttk.Label(content_frame, text="Задержка (µs):").grid(row=0, column=0, sticky="w", padx=2, pady=2)
    ...
    def update_block_data(self, block, key, value):
        ...
        try:
            if key in ["channel", "neg_current", "pos_current", "count", "delay_us"]:
    ...
    def _simulate_blocks_waveform(self, preview_channel: int = 0):
        ...
        for block in self.pattern_blocks:
            ...
            if btype == "current":
                ...
                delay_us = int(data.get("delay_us", data.get("count", 100)) or 0)
                if delay_us <= 0:
                    continue
                dur = float(delay_us)
    ...
    def generate_pattern_commands(self):
        ...
        for block in self.pattern_blocks:
            btype = block.get("type")
            if btype in ("step_size", "current"):
                continue
            if btype == "polarity":
                has_polarity = True
                ch = int(block["data"].get("channel", 0))
                positive = block["data"].get("positive", True)
                polarity_mask = (1 << ch) if positive else 0x0000
                ...
                pattern.append(self._format_pattern_add_raw(raw_on, f"R42 ON ch{ch}"))
            elif btype == "disable":
                raw_off = self._intan_raw_word(42, 0x0000, u_flag=True)
                pattern.append(self._format_pattern_add_raw(raw_off, "R42 OFF"))
            elif btype == "delay":
                delay_us = int(block["data"].get("delay_us", block["data"].get("count", 100)))
                pattern.append(f"PATTERN_ADD_DELAY_US {delay_us}")
            elif btype == "comment":
        ...
        if pattern:
            pattern = self._finalize_pattern_load_lines(
                self._pattern_lines_for_load([], pattern, finalize=False)
            )
